...
// no seeded/fictional fallback data. It falls through to a real empty state
// when there are none, rather than presenting fabricated content as verified
// customer activity.
...
interface Review {
  ...
  body: string;
  date_label: string;
  // Set by the API only when the reviewer proved order ownership (order number +
  // matching email). Drives the "Verified Buyer" badge — never assume it.
  verified?: boolean;
}
...
export function ReviewsSection() {
  ...
  const [form, setForm] = useState({ name: "", location: "", product: PRODUCT_NAMES[0], rating: 0, body: "", orderId: "", orderEmail: "" });
    ...
    <section id="reviews" className="py-[80px] md:py-[100px] bg-secondary overflow-hidden">
      <div className="max-w-[1024px] mx-auto px-4 md:px-6">
        ...
        >
          ...
          {/* Disabled — five hardcoded gold stars plus an unsubstantiated
              "4.9 average · 200+ verified orders" aggregate.
                ...
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {/* Only for reviews the API tied to a real order (order number +
                          matching email). Reviews without that link publish unbadged. */}
                      {review.verified && (
                        <span
            ...
            <>
              {!showForm && (
                <div className="flex flex-col items-center gap-3">
                  <p className="text-[15px] font-medium text-primary text-center">
                    {t("reviews_write_prompt")}
                  </p>
                  <button
                    onClick={() => setShowForm(true)}
                    className="group inline-flex items-center gap-2.5 rounded-full px-8 py-3.5 text-[15px] font-semibold text-white border-none cursor-pointer btn-physical btn-physical-accent"
                    style={{ backgroundColor: "var(--accent)" }}
                  >
                    <PenLine size={16} strokeWidth={2.2} style={{ pointerEvents: "none" }} />
                    <span style={{ pointerEvents: "none" }}>{t("reviews_write_btn")}</span>
                  </button>
                  <p className="text-[12px] text-center max-w-[340px] leading-relaxed" style={{ color: "var(--text-muted)" }}>
                    {t("reviews_write_sub")}
                  </p>
                </div>
              )}
              ...
              {showForm && (
                ...
                >
                  ...
                  {/* Optional order verification — both must match a real order for the
                      review to carry a "Verified Buyer" badge. */}
                  <div className="grid grid-cols-2 gap-3">
                    <input
                      placeholder={t("reviews_write_order")}
                      value={form.orderId}
                    ...
                    <input
                      type="email"
                      placeholder={t("reviews_write_order_email")}
                      value={form.orderEmail}
                      onChange={(e) => setForm((f) => ({ ...f, orderEmail: e.target.value }))}
                      className="rounded-[10px] px-4 py-2.5 text-[14px] outline-none col-span-2 md:col-span-1"
                      style={{ backgroundColor: "var(--bg-alt)", border: "1px solid var(--border)", color: "var(--text)" }}
                    />
                    <p className="col-span-2 text-[12px] leading-relaxed" style={{ color: "var(--text-muted)" }}>
                      {t("reviews_write_order_hint")}
                    </p>
                  </div>
